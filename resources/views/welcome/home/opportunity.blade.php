<!-- ======= Tech Section ======= -->
<section id="about" class="about section-bg">
    <div class="container" data-aos="fade-up">        
      <div class="section-title">
        <h2>Keuntungan yang kami berikan</h2></div>
      <div class="row">
        <div class="col-lg-6 pt-4 pt-lg-0 content d-flex flex-column justify-content-center" data-aos="fade-left" data-aos-delay="100">
            <ul>
                <li>
                  <i class="bx bi bi-headset"></i>
                  <div>
                    <h5>Responsif</h5>
                    <p>Respon cepat tindakan cepat jika terjadi masalah</p>
                  </div>
                </li>
                <li>
                  <i class="bx bi bi-shield-shaded"></i>
                  <div>
                    <h5>Internet sehat kominfo</h5>
                    <p>Internet Positif, aman dari konten hoax, pornografi & penipuan</p>
                  </div>
                </li>
                <li>
                  <i class="bx bi bi-wifi"></i>
                  <div>
                    <h5>Add On gratis</h5>
                    <p>Tambahan WiFi gratis</p>
                  </div>
                </li>
            </ul>
        </div>
        <div class="col-lg-6 pt-4 pt-lg-0 content d-flex flex-column justify-content-center" data-aos="fade-right" data-aos-delay="100">
            <ul>
                <li>
                  <i class="bx bi-play-circle"></i>
                  <div>
                    <h5>Media</h5>
                    <p>Bebas streaming & unlimited Quota</p>
                  </div>
                </li>
                <li>
                  <i class="bx bi bi-speedometer"></i>
                  <div>
                    <h5>Efisiensi tinggi</h5>
                    <p>Durable, bebas brebet , lemot dan mati mendadak</p>
                  </div>
                </li>
                <li>
                  <i class="bi bi-cash-coin"></i>
                  <div>
                    <h5>Hemat Biaya</h5>
                    <p>Ekonomis dan upgradable</p>
                  </div>
                </li>
            </ul>
        </div>
        <form method="GET" action="#">
            @csrf
            <button class="btn btn-primary" alt="Show Me" value="Show Me" style="align-content: center;">Selengkapnya <span class="bi bi-arrow-right" style="2rem;"></span></button>
        </form>
        {{-- <a href="#" class="btn btn-success" style="text-decoration: none; width: 10em;">Show Me <span class="bi bi-arrow-right"></span></a> --}}
        </div>
    </div>
    </div>
</section>
<!-- End Tech Section -->