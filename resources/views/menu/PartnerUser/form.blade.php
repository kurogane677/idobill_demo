<div class="row mt-4 d-flex justify-content-center">
  <div class="col-6">
    <div class="text-center mb-2">
      <img src="/storage/group_logo/{{$PartnerGroup->group_logo}}" alt="logo" style="width: 30%;">
    </div>
    <h5 class="text-center">Informasi Group</h5>
    <x-inputreadonly title="ID Group" ipname="ID_Group" value="{{$PartnerGroup->group_id}}" />
    <x-inputreadonly title="Nama" ipname="Nama_Group" value="{{$PartnerGroup->group_name}}" />
    <x-inputreadonly title="Email" ipname="Email_Group" value="{{$PartnerGroup->group_email}}" />
    <x-inputreadonly title="No. Pelayanan" ipname="NoHP_Group" value="{{$PartnerGroup->group_nohp_code . $PartnerGroup->group_no_hp}}" />
    <x-inputreadonly title="Alamat" ipname="Alamat_Group" value="{{$PartnerGroup->group_address}}" />
  </div>
  <div class="col-6">
    ...
    <!-- Nama -->
    @if (Request::is('*/edit'))
    <x-form-input title="Nama *" ipname="user_name" placeholder="" value="{{$PartnerUser->user_name}}" opsi="required autofocus" />
    @else
    ...
    <!-- Cotact No. -->
    @if (Request::is('*/edit'))
    <x-hpno-input title="Cotact No. *" codename="nohp_code" ipname="no_hp" code="{{$user->nohp_code}}" value="{{$user->no_hp}}" />
    @else
    <x-hpno-input title="Cotact No. *" codename="nohp_code" ipname="no_hp" code="+62" value="{{old('no_hp')}}" />
    @endif
    ...
    <footer class="blockquote-footer text-right">
      <small class="text-muted">
        terakhir diedit: <cite title="Source Title">
          {{ Carbon\Carbon::parse($PartnerUser->updated_at)->format('d M Y H:i:s') }}
          oleh
          {{$updated_by}}
        </cite>
      </small>
